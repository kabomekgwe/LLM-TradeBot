"""LSTM Training Script with Validation.
...
3. Creates sequences with DataPreprocessor
4. Trains BiLSTM with validation
5. Implements early stopping
6. Saves best model checkpoint

Training strategies from research:
...
def fetch_and_prepare_data(
    ...
    """Fetch historical data and prepare for LSTM training.

    Args:
        symbol: Trading pair (default: BTC/USDT)
        timeframe: Candle timeframe (default: 5m)
        limit: Number of candles to fetch (default: 10000)
        sequence_length: Sequence length for LSTM (default: 50)
    ...
    # In production, replace this with actual CCXT exchange fetch
    ...
    engineer = FeatureEngineer(
        windows=[5, 10, 20, 50],
        include_target=True,
        target_horizon=5  # 5-candle lookahead (like Phase 2)
    )
    ...
    # Create sequences (fit scaler on all data before split)
    ...
    # Split into train/validation/test (chronological, NO shuffle)
...
def train_epoch(
    ...
    Args:
        model: BiLSTM model
        train_loader: Training data loader (NO shuffle)
        criterion: Loss function (BCEWithLogitsLoss)
        optimizer: AdamW optimizer
        device: torch device (CPU for MacBook)
...
def validate(
    ...
    Args:
        model: BiLSTM model
        val_loader: Validation data loader (NO shuffle)
        criterion: Loss function (BCEWithLogitsLoss)
...
def main():
    ...
    # Force CPU device (MacBook compatible)
    device = torch.device('cpu')
    ...
    # CRITICAL: NO SHUFFLE (prevents data leakage)
    ...
    criterion = nn.BCEWithLogitsLoss()  # Binary cross-entropy with logits (numerically stable)
    optimizer = optim.AdamW(model.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)

    # Learning rate scheduler (ReduceLROnPlateau from research)
    ...
    patience_counter = 0
    best_model_path = Path('trading/ml/models/deep_learning/lstm_model.pth')
