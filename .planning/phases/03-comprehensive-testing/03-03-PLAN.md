---
phase: 03-comprehensive-testing
plan: 03
type: execute
---

<objective>
Implement ML model tests and integration tests for full pipeline validation.

Purpose: Validate LightGBM training/prediction behavior and end-to-end system integration.
Output: ML model tests (training, prediction, feature extraction) and integration tests (full decision pipeline).
</objective>

<execution_context>
~/.claude/get-shit-done/workflows/execute-phase.md
~/.claude/get-shit-done/templates/summary.md
</execution_context>

<context>
@.planning/PROJECT.md
@.planning/ROADMAP.md
@.planning/STATE.md
@.planning/phases/03-comprehensive-testing/DISCOVERY.md
@.planning/phases/03-comprehensive-testing/03-01-PLAN.md
@.planning/phases/03-comprehensive-testing/03-02-PLAN.md

**Codebase constraints:**
- Python 3.7+ with asyncio
- pytest-asyncio 1.3.0 with auto mode
- LightGBM for ML predictions (binary classification)
- Full pipeline: DataSync → QuantAnalyst → Predict/Bull/Bear → DecisionCore → RiskManager → Executor

**From codebase analysis (CONCERNS.md):**
- **Current issue**: No tests for ML training or prediction logic
- **Impact**: Can't verify model produces valid predictions or that training works
- **Integration risk**: No validation that agents work together correctly in pipeline

**From DISCOVERY.md:**
- ML testing: Unit tests for training, prediction tests, feature extraction validation, mock model for agent tests
- Integration testing: Full pipeline test from data fetch to final decision
- LightGBM behavioral testing: Verify probability ranges, confidence scaling, feature alignment
- Mock trained model pattern: Use Mock for testing agents without full training

**Prior work:**
- Plan 03-01 completed - Test infrastructure and fixtures available (mock_lightgbm_model fixture)
- Plan 03-02 completed - Agent and risk management tests complete
- Phase 2 completed - LightGBM training script and PredictAgent integration

**Why this matters:**
ML predictions influence trading decisions worth real money. Without tests, we can't verify the model trains correctly, produces valid predictions, or that features match the training schema. Integration tests ensure the 8-agent system works together correctly (not just individual agents).
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create ML model training and prediction tests</name>
  <files>trading/tests/test_ml.py</files>
  <action>
Test LightGBM model training, prediction, and feature extraction logic.

**1. Create `trading/tests/test_ml.py`:**

```python
"""
Machine learning model tests for LightGBM training and prediction.
Tests model training, prediction validity, and feature extraction.
"""

import pytest
import numpy as np
import lightgbm as lgb
import os
import tempfile

from trading.ml.train_lightgbm import (
    calculate_features,
    create_labels,
    fetch_historical_data
)


# === Feature Extraction Tests ===

@pytest.mark.unit
@pytest.mark.ml
def test_calculate_features_returns_correct_shape(ohlcv_factory):
    """Test feature extraction returns correct number of features."""
    # Arrange
    ohlcv = ohlcv_factory(num_candles=100)

    # Act
    features = calculate_features(ohlcv)

    # Assert - Should return 8 features: RSI, MACD (3), BB (3), returns (1)
    assert features.shape[1] == 8
    assert features.shape[0] == 100


@pytest.mark.unit
@pytest.mark.ml
def test_calculate_features_handles_short_sequences(ohlcv_factory):
    """Test feature extraction handles short sequences (fills with NaN for early periods)."""
    # Arrange - Very short sequence
    ohlcv = ohlcv_factory(num_candles=15)  # Less than indicators need

    # Act
    features = calculate_features(ohlcv)

    # Assert - Should return features but early rows will be NaN
    assert features.shape[0] == 15
    assert np.isnan(features[0]).any()  # First row should have NaN (not enough history)


@pytest.mark.unit
@pytest.mark.ml
def test_calculate_features_produces_valid_values(ohlcv_factory):
    """Test feature extraction produces valid numeric values (no inf, valid ranges)."""
    # Arrange
    ohlcv = ohlcv_factory(num_candles=100, trend="uptrend")

    # Act
    features = calculate_features(ohlcv)

    # Assert - After warmup period, values should be valid
    valid_features = features[30:]  # Skip warmup period
    assert not np.isinf(valid_features).any()  # No infinity values
    assert not np.isnan(valid_features).all()  # Not all NaN

    # RSI should be between 0 and 100 (first column)
    rsi_values = valid_features[:, 0]
    rsi_values = rsi_values[~np.isnan(rsi_values)]
    assert (rsi_values >= 0).all() and (rsi_values <= 100).all()


# === Label Creation Tests ===

@pytest.mark.unit
@pytest.mark.ml
def test_create_labels_binary_classification(ohlcv_factory):
    """Test label creation produces binary labels (0 or 1)."""
    # Arrange
    ohlcv = ohlcv_factory(num_candles=100, trend="uptrend")

    # Act
    labels = create_labels(ohlcv, lookahead=5)

    # Assert
    assert set(np.unique(labels)).issubset({0, 1})  # Only 0 or 1
    assert len(labels) == 95  # 100 - 5 lookahead


@pytest.mark.unit
@pytest.mark.ml
def test_create_labels_uptrend_produces_mostly_ones(ohlcv_factory):
    """Test label creation in uptrend produces mostly 1s (price going up)."""
    # Arrange
    ohlcv = ohlcv_factory(num_candles=100, trend="uptrend", volatility=0.01)

    # Act
    labels = create_labels(ohlcv, lookahead=5)

    # Assert - Uptrend should have more 1s than 0s
    ones_ratio = np.sum(labels) / len(labels)
    assert ones_ratio > 0.5  # More than 50% should be 1s


@pytest.mark.unit
@pytest.mark.ml
def test_create_labels_downtrend_produces_mostly_zeros(ohlcv_factory):
    """Test label creation in downtrend produces mostly 0s (price going down)."""
    # Arrange
    ohlcv = ohlcv_factory(num_candles=100, trend="downtrend", volatility=0.01)

    # Act
    labels = create_labels(ohlcv, lookahead=5)

    # Assert - Downtrend should have more 0s than 1s
    zeros_ratio = 1 - (np.sum(labels) / len(labels))
    assert zeros_ratio > 0.5  # More than 50% should be 0s


# === Model Training Tests ===

@pytest.mark.slow
@pytest.mark.ml
def test_lightgbm_model_training(ohlcv_factory):
    """Test LightGBM model trains successfully with synthetic data."""
    # Arrange - Generate synthetic training data
    ohlcv = ohlcv_factory(num_candles=200, trend="uptrend")
    features = calculate_features(ohlcv)
    labels = create_labels(ohlcv, lookahead=5)

    # Remove NaN rows
    valid_mask = ~np.isnan(features).any(axis=1)
    features = features[valid_mask]
    labels = labels[valid_mask[:len(labels)]]

    # Align
    min_len = min(len(features), len(labels))
    features = features[:min_len]
    labels = labels[:min_len]

    # Create dataset
    train_data = lgb.Dataset(features, label=labels)

    # Act - Train model
    params = {
        'objective': 'binary',
        'metric': 'binary_logloss',
        'boosting_type': 'gbdt',
        'num_leaves': 31,
        'learning_rate': 0.05,
        'verbose': -1
    }

    bst = lgb.train(
        params,
        train_data,
        num_boost_round=10,  # Quick training for test
        valid_sets=[train_data]
    )

    # Assert
    assert bst is not None
    assert bst.num_trees() == 10  # Should have 10 trees


@pytest.mark.slow
@pytest.mark.ml
def test_lightgbm_model_predictions_valid_range(ohlcv_factory):
    """Test trained model produces predictions in valid probability range [0, 1]."""
    # Arrange - Train a quick model
    ohlcv = ohlcv_factory(num_candles=200, trend="uptrend")
    features = calculate_features(ohlcv)
    labels = create_labels(ohlcv, lookahead=5)

    valid_mask = ~np.isnan(features).any(axis=1)
    features = features[valid_mask]
    labels = labels[valid_mask[:len(labels)]]
    min_len = min(len(features), len(labels))
    features = features[:min_len]
    labels = labels[:min_len]

    train_data = lgb.Dataset(features, label=labels)
    params = {'objective': 'binary', 'verbose': -1}
    bst = lgb.train(params, train_data, num_boost_round=10)

    # Act - Make predictions
    predictions = bst.predict(features[:10])  # Test on first 10 samples

    # Assert - Probabilities should be between 0 and 1
    assert (predictions >= 0).all()
    assert (predictions <= 1).all()
    assert len(predictions) == 10


# === Model Persistence Tests ===

@pytest.mark.unit
@pytest.mark.ml
def test_lightgbm_model_save_load(ohlcv_factory):
    """Test LightGBM model can be saved and loaded."""
    # Arrange - Train a quick model
    ohlcv = ohlcv_factory(num_candles=200)
    features = calculate_features(ohlcv)
    labels = create_labels(ohlcv, lookahead=5)

    valid_mask = ~np.isnan(features).any(axis=1)
    features = features[valid_mask]
    labels = labels[valid_mask[:len(labels)]]
    min_len = min(len(features), len(labels))
    features = features[:min_len]
    labels = labels[:min_len]

    train_data = lgb.Dataset(features, label=labels)
    params = {'objective': 'binary', 'verbose': -1}
    bst = lgb.train(params, train_data, num_boost_round=5)

    # Act - Save and load model
    with tempfile.NamedTemporaryFile(suffix='.txt', delete=False) as tmp:
        model_path = tmp.name

    try:
        bst.save_model(model_path)
        loaded_model = lgb.Booster(model_file=model_path)

        # Make predictions with both models
        original_pred = bst.predict(features[:5])
        loaded_pred = loaded_model.predict(features[:5])

        # Assert - Predictions should be identical
        np.testing.assert_array_almost_equal(original_pred, loaded_pred, decimal=6)

    finally:
        if os.path.exists(model_path):
            os.unlink(model_path)


# === PredictAgent Feature Alignment Tests ===

@pytest.mark.unit
@pytest.mark.ml
def test_predict_agent_features_match_training_schema():
    """Test PredictAgent extracts features in same order as training script."""
    # Arrange - Mock indicators from QuantAnalyst
    indicators = {
        'rsi': {'value': 65.5},
        'macd': {'macd': 0.5, 'signal': 0.3, 'histogram': 0.2},
        'bollinger': {'upper': 30100, 'middle': 30000, 'lower': 29900}
    }

    # Expected feature order from training script:
    # [RSI, MACD, Signal, Histogram, Upper BB, Middle BB, Lower BB, Returns]
    expected_order = [
        65.5,   # RSI
        0.5,    # MACD
        0.3,    # Signal
        0.2,    # Histogram
        30100,  # Upper BB
        30000,  # Middle BB
        29900,  # Lower BB
        0.0     # Returns placeholder
    ]

    # Act - Extract features using PredictAgent logic
    features = np.array([[
        indicators['rsi']['value'],
        indicators['macd']['macd'],
        indicators['macd']['signal'],
        indicators['macd']['histogram'],
        indicators['bollinger']['upper'],
        indicators['bollinger']['middle'],
        indicators['bollinger']['lower'],
        0.0  # Price returns placeholder
    ]])

    # Assert - Feature order must match training
    np.testing.assert_array_equal(features[0], expected_order)


@pytest.mark.unit
@pytest.mark.ml
def test_predict_agent_confidence_scaling():
    """Test PredictAgent scales probability to confidence correctly."""
    # Test cases: (probability, expected_confidence)
    # Formula: confidence = abs(prob - 0.5) * 2
    test_cases = [
        (0.5, 0.0),   # Neutral probability = 0 confidence
        (0.75, 0.5),  # 75% up = 50% confidence
        (1.0, 1.0),   # 100% up = 100% confidence
        (0.25, 0.5),  # 25% up (75% down) = 50% confidence
        (0.0, 1.0),   # 0% up (100% down) = 100% confidence
    ]

    for prob, expected_conf in test_cases:
        # Act
        confidence = abs(prob - 0.5) * 2

        # Assert
        assert abs(confidence - expected_conf) < 0.001, \
            f"Prob {prob} should give confidence {expected_conf}, got {confidence}"
```

**What to avoid:**
- Don't test LightGBM internals (test our usage of it, not the library itself)
- Don't train large models in tests (use num_boost_round=5-10 for speed)
- Don't skip feature alignment tests (mismatched features = garbage predictions)
- Don't hardcode expected predictions (models are stochastic, test ranges)

**Why these choices:**
- Feature shape tests catch schema changes early
- Binary label tests validate training data correctness
- Model persistence tests ensure models can be saved/loaded reliably
- Feature alignment tests prevent critical bug: features in wrong order
- Confidence scaling tests validate probability-to-confidence conversion
- Slow marker allows skipping expensive training tests during rapid iteration
  </action>
  <verify>
1. All tests pass: `pytest trading/tests/test_ml.py -v` passes
2. Slow tests work: `pytest -m slow` runs training tests
3. Feature alignment verified: test_predict_agent_features_match_training_schema passes
4. Model training works: test_lightgbm_model_training completes without errors
  </verify>
  <done>
- Complete ML testing suite for LightGBM
- Feature extraction tests validate shape and value ranges
- Label creation tests verify binary classification correctness
- Model training tests ensure successful training with synthetic data
- Prediction tests validate probability ranges [0, 1]
- Model persistence tests verify save/load functionality
- Feature alignment tests prevent schema mismatch bugs
- Confidence scaling tests validate probability conversion
  </done>
</task>

<task type="auto">
  <name>Task 2: Create integration tests for full pipeline</name>
  <files>trading/tests/test_integration.py</files>
  <action>
Test complete decision pipeline from data fetch to final decision.

**1. Create `trading/tests/test_integration.py`:**

```python
"""
Integration tests for full agent pipeline.
Tests end-to-end flow: DataSync → QuantAnalyst → Predict/Bull/Bear → DecisionCore → RiskManager → Executor.
"""

import pytest
import pytest_asyncio
from unittest.mock import AsyncMock, Mock

from trading.agents.data_sync import DataSyncAgent
from trading.agents.quant_analyst import QuantAnalystAgent
from trading.agents.predict import PredictAgent
from trading.agents.bull import BullAgent
from trading.agents.bear import BearAgent
from trading.agents.risk_manager import RiskManagerAgent
from trading.agents.executor import ExecutorAgent
from trading.core.decision import DecisionCore
from trading.config import TradingConfig


# === Full Pipeline Integration Tests ===

@pytest.mark.asyncio
@pytest.mark.integration
async def test_full_pipeline_bullish_scenario(
    mock_exchange,
    ohlcv_factory,
    mock_lightgbm_model
):
    """Test complete pipeline with bullish market scenario."""
    # Arrange - Setup bullish market data
    ohlcv_data = ohlcv_factory(trend="uptrend", num_candles=100, volatility=0.01)
    mock_exchange.fetch_ohlcv.return_value = ohlcv_data
    mock_exchange.fetch_balance.return_value = {
        'USDT': {'free': 10000, 'used': 0, 'total': 10000}
    }

    config = TradingConfig(symbol="BTC/USDT", max_position_size=0.1)

    # Create agents
    data_sync = DataSyncAgent(config)
    quant_analyst = QuantAnalystAgent(config)
    predict = PredictAgent(config)
    predict.model = mock_lightgbm_model  # Inject mock model
    bull = BullAgent(config)
    bear = BearAgent(config)
    risk_manager = RiskManagerAgent(config)
    executor = ExecutorAgent(config)
    decision_core = DecisionCore(config)

    # Act - Execute full pipeline
    context = {}

    # Step 1: Fetch data
    context = await data_sync.execute({'exchange': mock_exchange})

    # Step 2: Calculate indicators
    quant_result = await quant_analyst.execute(context)
    context['quant_analyst'] = quant_result

    # Step 3: Get agent votes
    predict_result = await predict.execute(context)
    bull_result = await bull.execute(context)
    bear_result = await bear.execute(context)

    # Step 4: Make decision
    decision = decision_core.vote([bull_result, bear_result, predict_result])
    context['decision'] = decision

    # Step 5: Apply risk management
    context['balance'] = mock_exchange.fetch_balance.return_value
    context['daily_pnl'] = 0  # No losses today
    context['current_price'] = 30000
    risk_result = await risk_manager.execute(context)
    context['decision'] = risk_result

    # Step 6: Execute trade
    final_result = await executor.execute({'decision': context['decision'], 'exchange': mock_exchange})

    # Assert - In bullish scenario, should produce buy signal
    assert decision['action'] in ['buy', 'hold']  # Bull agents should vote buy
    assert 'confidence' in decision
    assert decision['confidence'] >= 0.0

    # If risk manager allowed trade, executor should place order
    if risk_result['action'] == 'buy':
        mock_exchange.create_order.assert_called()


@pytest.mark.asyncio
@pytest.mark.integration
async def test_full_pipeline_bearish_scenario(
    mock_exchange,
    ohlcv_factory,
    mock_lightgbm_model
):
    """Test complete pipeline with bearish market scenario."""
    # Arrange - Setup bearish market data
    ohlcv_data = ohlcv_factory(trend="downtrend", num_candles=100, volatility=0.01)
    mock_exchange.fetch_ohlcv.return_value = ohlcv_data
    mock_exchange.fetch_balance.return_value = {
        'USDT': {'free': 10000, 'used': 0, 'total': 10000}
    }

    config = TradingConfig(symbol="BTC/USDT", max_position_size=0.1)

    # Create agents
    data_sync = DataSyncAgent(config)
    quant_analyst = QuantAnalystAgent(config)
    predict = PredictAgent(config)
    predict.model = mock_lightgbm_model
    bull = BullAgent(config)
    bear = BearAgent(config)
    decision_core = DecisionCore(config)

    # Act - Execute pipeline up to decision
    context = {}
    context = await data_sync.execute({'exchange': mock_exchange})
    quant_result = await quant_analyst.execute(context)
    context['quant_analyst'] = quant_result

    predict_result = await predict.execute(context)
    bull_result = await bull.execute(context)
    bear_result = await bear.execute(context)

    decision = decision_core.vote([bull_result, bear_result, predict_result])

    # Assert - In bearish scenario, bear agents should have influence
    assert decision['action'] in ['sell', 'hold']  # Likely sell or hold
    assert 'confidence' in decision


@pytest.mark.asyncio
@pytest.mark.integration
async def test_risk_manager_vetoes_dangerous_trade(
    mock_exchange,
    ohlcv_factory,
    mock_lightgbm_model
):
    """Test risk manager vetoes trade when max loss limit exceeded."""
    # Arrange
    ohlcv_data = ohlcv_factory(trend="uptrend", num_candles=100)
    mock_exchange.fetch_ohlcv.return_value = ohlcv_data
    mock_exchange.fetch_balance.return_value = {
        'USDT': {'free': 10000, 'used': 0, 'total': 10000}
    }

    config = TradingConfig(
        symbol="BTC/USDT",
        max_daily_loss=500,  # $500 max loss
        max_position_size=0.1
    )

    # Create agents
    data_sync = DataSyncAgent(config)
    quant_analyst = QuantAnalystAgent(config)
    bull = BullAgent(config)
    bear = BearAgent(config)
    predict = PredictAgent(config)
    predict.model = mock_lightgbm_model
    decision_core = DecisionCore(config)
    risk_manager = RiskManagerAgent(config)
    executor = ExecutorAgent(config)

    # Act - Execute pipeline with exceeded loss limit
    context = {}
    context = await data_sync.execute({'exchange': mock_exchange})
    quant_result = await quant_analyst.execute(context)
    context['quant_analyst'] = quant_result

    predict_result = await predict.execute(context)
    bull_result = await bull.execute(context)
    bear_result = await bear.execute(context)

    decision = decision_core.vote([bull_result, bear_result, predict_result])
    context['decision'] = decision

    # Simulate exceeded daily loss
    context['balance'] = mock_exchange.fetch_balance.return_value
    context['daily_pnl'] = -600  # Lost $600 (exceeded $500 limit)
    context['current_price'] = 30000

    risk_result = await risk_manager.execute(context)
    context['decision'] = risk_result

    final_result = await executor.execute({'decision': context['decision'], 'exchange': mock_exchange})

    # Assert - Risk manager should veto trade
    assert risk_result['action'] == 'hold'
    assert 'max loss' in risk_result['reason'].lower()
    mock_exchange.create_order.assert_not_called()


@pytest.mark.asyncio
@pytest.mark.integration
async def test_pipeline_handles_missing_indicators_gracefully(
    mock_exchange,
    ohlcv_factory
):
    """Test pipeline handles missing indicator data gracefully (fail-safe)."""
    # Arrange - Setup data with insufficient candles for indicators
    ohlcv_data = ohlcv_factory(num_candles=10)  # Too few for indicators
    mock_exchange.fetch_ohlcv.return_value = ohlcv_data

    config = TradingConfig(symbol="BTC/USDT")

    # Create agents
    data_sync = DataSyncAgent(config)
    quant_analyst = QuantAnalystAgent(config)
    bull = BullAgent(config)
    bear = BearAgent(config)
    decision_core = DecisionCore(config)

    # Act - Execute pipeline
    context = {}
    context = await data_sync.execute({'exchange': mock_exchange})
    quant_result = await quant_analyst.execute(context)
    context['quant_analyst'] = quant_result

    bull_result = await bull.execute(context)
    bear_result = await bear.execute(context)

    # Predict agent without model (fallback)
    predict_result = {'prediction': 0.0, 'confidence': 0.0}

    decision = decision_core.vote([bull_result, bear_result, predict_result])

    # Assert - Should fail safe (likely hold decision with low confidence)
    assert decision['action'] in ['buy', 'sell', 'hold']
    assert decision['confidence'] >= 0.0  # Valid confidence


@pytest.mark.asyncio
@pytest.mark.integration
async def test_decision_core_weights_votes_correctly(
    mock_exchange,
    ohlcv_factory,
    mock_lightgbm_model
):
    """Test DecisionCore combines agent votes with proper weighting."""
    # Arrange
    ohlcv_data = ohlcv_factory(trend="uptrend", num_candles=100)
    mock_exchange.fetch_ohlcv.return_value = ohlcv_data

    config = TradingConfig(symbol="BTC/USDT")

    # Create agents
    data_sync = DataSyncAgent(config)
    quant_analyst = QuantAnalystAgent(config)
    predict = PredictAgent(config)
    predict.model = mock_lightgbm_model
    bull = BullAgent(config)
    bear = BearAgent(config)
    decision_core = DecisionCore(config)

    # Act - Get agent votes
    context = {}
    context = await data_sync.execute({'exchange': mock_exchange})
    quant_result = await quant_analyst.execute(context)
    context['quant_analyst'] = quant_result

    predict_result = await predict.execute(context)
    bull_result = await bull.execute(context)
    bear_result = await bear.execute(context)

    # Get votes
    votes = [bull_result, bear_result, predict_result]
    decision = decision_core.vote(votes)

    # Assert - Decision should aggregate votes
    assert 'action' in decision
    assert 'confidence' in decision
    assert decision['action'] in ['buy', 'sell', 'hold']
    assert 0.0 <= decision['confidence'] <= 1.0

    # Verify all agents contributed (check that votes were considered)
    # Decision should reflect the aggregate of all agent opinions
    assert len(votes) == 3  # All 3 agents voted
```

**What to avoid:**
- Don't mock everything (use real agents, mock only external systems like exchange)
- Don't test individual agent behavior (that's unit tests, test interactions here)
- Don't hardcode expected outcomes (market scenarios have nuanced results)
- Don't skip edge cases (missing data, insufficient balance, exceeded limits)

**Why these choices:**
- Full pipeline tests validate agent interactions, not just individual behavior
- Bullish/bearish scenarios test realistic market conditions
- Risk veto test validates safety-critical risk management integration
- Missing data test ensures fail-safe behavior across pipeline
- Decision weighting test validates ensemble voting logic
- Integration tests catch bugs that unit tests miss (coordination failures)
  </action>
  <verify>
1. All tests pass: `pytest trading/tests/test_integration.py -v` passes
2. Integration marker works: `pytest -m integration` runs only integration tests
3. Pipeline executes: Verify full pipeline from DataSync to Executor completes
4. Risk veto works: test_risk_manager_vetoes_dangerous_trade passes
  </verify>
  <done>
- Complete integration test suite for full pipeline
- Bullish/bearish scenario tests validate agent interactions
- Risk veto integration test ensures safety-critical behavior
- Missing data test validates fail-safe behavior
- Decision weighting test validates ensemble voting
- End-to-end validation from data fetch to trade execution
- Pipeline coordination bugs caught by integration tests
  </done>
</task>

</tasks>

<verification>
Before declaring plan complete:
- [ ] ML tests created for training and prediction
- [ ] Feature extraction tests validate shape and ranges
- [ ] Label creation tests verify binary classification
- [ ] Model persistence tests verify save/load
- [ ] Feature alignment tests prevent schema mismatch
- [ ] Integration tests validate full pipeline (DataSync → Executor)
- [ ] Bullish/bearish scenarios tested
- [ ] Risk veto integration tested
- [ ] All tests pass with pytest -v
</verification>

<success_criteria>

- Both tasks completed successfully
- ML model training and prediction tests complete
- Feature extraction and label creation validated
- Model persistence verified
- Full pipeline integration tests complete
- Bullish/bearish scenarios tested
- Risk management integration validated
- **Phase 3 (Comprehensive Testing) COMPLETE**
- Ready for Phase 4 (Decision Transparency & Error Handling)
</success_criteria>

<output>
After completion, create `.planning/phases/03-comprehensive-testing/03-03-SUMMARY.md`:

# Phase 3 Plan 3: ML & Integration Tests Summary

**[One-liner describing what shipped]**

## Accomplishments

- Created comprehensive ML testing suite (training, prediction, features)
- Created full pipeline integration tests (DataSync → Executor)
- [Additional accomplishments]

## Files Created/Modified

- `trading/tests/test_ml.py` - LightGBM training and prediction tests
- `trading/tests/test_integration.py` - Full pipeline integration tests

## Decisions Made

- Slow marker for expensive ML training tests
- Feature alignment tests prevent schema mismatch bugs
- Integration tests use real agents (mock only external systems)
- [Other decisions]

## Deviations from Plan

[Document any deviations using deviation rules, or "None"]

## Issues Encountered

[Problems and resolutions, or "None"]

## Verification Results

[Results of verification checklist]

## Next Step

**Phase 3 Complete!** All testing objectives achieved.

Ready for Phase 4: Decision Transparency & Error Handling
</output>
